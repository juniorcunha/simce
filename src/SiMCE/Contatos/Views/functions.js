SiMCE.functions = {
	...
	removeForm: function() {
		...
		// Obtem os dados selecionados do grid para exibir o formulário
		bootbox.confirm("Tem certeza que deseja remover o contato " + SiMCE.functions.data.nome + "?", function(result) {
				...
				SiMCE.ajaxRequest(
					...
					function( data, textStatus, jqXHR ) {
						...
						if ( data.success === true ) {
							...
							SiMCE.successMessage("Contato removido com sucesso!");
						}
	...
	submit: function() {
		...
		SiMCE.ajaxRequest(
			...
			$("#form-contatos").serialize() + "&operacao=" + SiMCE.getOperacao(),
			function( data, textStatus, jqXHR ) {
				...
				if ( data.success === true ) {
					...
					// Mensagem para o usuário
					SiMCE.successMessage("Contato cadastrado/atualizado com sucesso!");
	...
	addAlocation: function() {
		...
		if (ident == '') {
			...
			SiMCE.ajaxRequest(
				...
				"operacao=" + SiMCE.getOperacao() + "&recurso=" + $('select[id=recurso-combo]').val() + "&inicio=" + inicio + "&fim=" + fim + "&ident=" + ident,
				function( data, textStatus, jqXHR ) {
					if (data.success === true) {
	...
	updateAlocation: function( index ) {
		...
		if (nObj.identificacao == '') {
			...
			SiMCE.ajaxRequest(
				"POST",
				SiMCE.moduleURL + "checkAlocation/",
				"operacao=" + SiMCE.getOperacao() + "&recurso=" + $('select[id=recurso-combo]').val() + "&inicio=" + nObj.inicio + "&fim=" + nObj.fim + "&id=" + nObj.id,
				function( data, textStatus, jqXHR ) {
					if (data.success === true) {
